#!/usr/bin/env python3
"""
Alwadi Solar Cell Tenders Monitor — Smart Contextual Analyzer
يحلل مناقصات الخلايا الشمسية والطاقة المتجددة، يفلتر المشاريع المستهدفة
...
CONFIG = {
    ...
    "from_name":  "Alwadi Solar Cell Tenders Monitor",
    "batch_size": 10,
...
STATE_FILE = "/home/solutions/.openclaw/solar_state.json"
LOG_FILE   = "/home/solutions/.openclaw/solar_log.txt"
CONTACTS_FILE = "/home/solutions/.openclaw/solar_contacts.csv"
...
class SolarAnalyzer:
    """
    محلل سياقي للطاقة الشمسية — يقرأ النص الكامل للمناقصة ويحكم:
    1. هل هي مشروع خلايا شمسية/طاقة متجددة؟
    2. ما نوع المشروع (تطبيق، حجم، موقع)؟
    3. ما الحل المقترح؟
    4. ما القيمة المقدرة؟
    """

    # --- Solar positive signals (context-aware) ---
    SOLAR_SIGNALS = {
        "خلايا شمسية":         10, "solar cell":           10, "pv cell":            10,
        "الخلايا الشمسية":     10, "solar power":          9,  "solar powered":       9,
        "solar system":        10, "نظام شمسي":             10, "نظام الطاقة الشمسية": 10,
        "طاقة شمسية":         10, "renewable energy":      9,  "الطاقة المتجددة":    9,
        "renewables":          8,  "طاقات متجددة":          9,  "renewable energies":  9,
        "photovoltaic":        10, "pv system":             10, "pv module":           10,
        "solar panel":         10, "لوح شمسي":              10, "الواح شمسية":        10,
        "solar installation":  9,  "تركيب شمسي":            9,  "solar powered water": 9,
        "solar powered water pumping": 10, "نظام الضخ الشمسي": 10,
        "solar supply install": 9,  "solar supply and install": 9,  # More specific than generic
        "water pumping":       6,  "ضخ المياه":             6,  "بئر":               4,
        "borehole":            6,  "well":                  5,  "water well":         6,
        "grid connected":      8,  "grid-connected solar":  10, "شبكة كهربائية":    6,
        "off-grid":            8,  "hybrid system":         8,  "نظام هجين":         8,
        "battery":             7,  "بطارية":                7,  "energy storage":     8,
        "inverter":            8,  "محول كهربائي":          8,  "charge controller": 8,
        "mounting":            7,  "mounting structure":    7,  "solar tracker":      8,
        "epc":                 8,  "engineering procurement": 8,
        "prequalification":    7,  "تصنيف الموردين":        7,
        "kw":                  6,  "kilowatt":              6,  "megawatt":           6,
        "mw":                  6,  "وات":                  6,
        "renewable":           7,  "متجدد":                 7,
        "solarization":        10, "تمويل الطاقة":          6,
        "street light":        8,  "إضاءة الشوارع":         8,  "solar street light": 10,
        "solar light":         9,  "إضاءة شمسية":          9,
        "healthcare":          6,  "school":                6,  "medical facility":   6,
        "sustainable":         6,  "environmental":         6,
        "climate change":      6,  "renewable technology":  7,
        "ملف مرفق":           3,  # File attachment indicator
    }

    # --- Strong non-solar signals that kill solar classification ---
    NOT_SOLAR_SIGNALS = {
        "office equipment": -8, "مستلزمات مكاتب": -8,
        "stationery": -10, "قرطاسية": -10, "office supplies": -10, "أدوات مكتبية": -10,
        "furniture": -9, "أثاث": -9, "shelves": -9, "رفوف": -9, "metal shelves": -10,
        "workshop equipment": -9, "معدات الورش": -9, "ورشة": -8,
        "maintenance": -8, "صيانة": -8, "repair": -7, "إصلاح": -7,
        "security guard": -10, "حارسة أمنية": -10,
        "construction": -6, "أعمال إنشائية": -6,
        "vehicles": -9, "مركبات": -9,
        "food": -8, "مواد غذائية": -8,
        "medical equipment": -8, "معدات طبية": -8,
        "it system": -6, "نظام معلومات": -6,
        "network": -6, "شبكة بيانات": -6,
    }

    # --- Target sectors ---
    SECTORS = {
        "المشاريع الحكومية": {
            "keywords": ["حكومة", "وزارة", "government", "municipal", "مجلس بلدي",
                        "صحة", "hospital", "school", "مدرسة", "عام"],
            "color": "#1F4E78", "icon": "🏛️"
        },
        "المشاريع الخاصة": {
            "keywords": ["شركة", "company", "private", "خاص", "تجاري", "business"],
            "color": "#00AA00", "icon": "🏢"
        },
        "مشاريع المياه": {
            "keywords": ["مياه", "water", "بئر", "well", "ضخ", "pumping", "خزان",
                        "بئر مياه", "محطة مياه"],
            "color": "#4472C4", "icon": "💧"
        },
        "الطاقة والكهرباء": {
            "keywords": ["كهرباء", "electricity", "power", "شبكة", "network", "توزيع"],
            "color": "#FFC000", "icon": "⚡"
        },
    }
    ...
    SOLUTIONS = [
        {
            "triggers":   ["solar water pump", "solar powered water", "ضخ شمسي", "نظام ضخ شمسي",
                          "water pumping system", "مضخة شمسية"],
            "solution":   "نظام الضخ الشمسي — Solar Water Pumping Systems (10-37kW)",
            "value_range": (50_000, 500_000),
        },
        {
            "triggers":   ["solar street light", "إضاءة شمسية", "solar light", "street light",
                          "street lights", "solar powered light"],
            "solution":   "أنظمة إضاءة الشوارع الشمسية — Solar Street Lights with Batteries & Poles",
            "value_range": (30_000, 300_000),
        },
        {
            "triggers":   ["grid connected", "grid-connected", "شبكة متصلة", "محطة توليد",
                          "power plant", "solar power plant"],
            "solution":   "محطات الطاقة الشمسية المتصلة بالشبكة — Grid-Connected Solar Plants (1-5MW)",
            "value_range": (500_000, 10_000_000),
        },
        {
            "triggers":   ["pv system", "photovoltaic system", "نظام فوتوفولطائي",
                          "solar system", "نظام شمسي", "solar installation"],
            "solution":   "نظام الطاقة الشمسية الكامل — Complete PV System (Design + Supply + Install)",
            "value_range": (50_000, 2_000_000),
        },
        {
            "triggers":   ["off-grid", "hybrid", "standalone", "nظام هجين", "نظام مستقل",
                          "battery storage", "تخزين الطاقة"],
            "solution":   "أنظمة شمسية هجينة ومستقلة — Off-Grid & Hybrid Solar Systems with Battery",
            "value_range": (40_000, 1_000_000),
        },
        {
            "triggers":   ["epc", "engineering procurement", "هندسة وتركيب", "المقاول",
                          "supply and installation"],
            "solution":   "خدمات EPC الشاملة — Engineering, Procurement & Construction",
            "value_range": (100_000, 5_000_000),
        },
        {
            "triggers":   ["renewable", "متجدد", "sustainable", "climate", "green energy"],
            "solution":   "مشروع طاقة متجددة ومستدامة — Renewable Energy Project",
    ...
    def analyze(self, title: str, full_text: str) -> dict | None:
        ...
        🇱🇾 يفلتر مشاريع الطاقة الليبية فقط
        """
        ...
        # --- 1. حساب نقاط الطاقة الشمسية ---
        solar_score = 0
        for kw, score in self.SOLAR_SIGNALS.items():
            if kw.lower() in combined:
                solar_score += score

        # --- 2. خصم نقاط non-solar ---
        not_solar_score = 0
        for kw, penalty in self.NOT_SOLAR_SIGNALS.items():
            if kw.lower() in combined:
                not_solar_score += penalty

        net_score = solar_score + not_solar_score

        # رفض إذا النتيجة غير كافية — مرن أكثر لاكتشاف مشاريع الطاقة
        ...
        if not matched_solutions:
            matched_solutions = ["حل طاقة شمسية متكامل — يحتاج دراسة تفصيلية"]
    ...
    def _estimate_value_from_description(self, text: str) -> tuple[int, str]:
        """تقدير القيمة بناءً على المساحة والمكونات والنوع مع خفض 35%"""
        # استخراج الطاقة (kW, MW)
        power_match = re.search(r'(\d+)\s*(?:kw|kilowatt|mw|megawatt)', text, re.IGNORECASE)
        power_kw = 0
        if power_match:
            power_num = int(power_match.group(1))
            if 'mw' in power_match.group(0).lower():
                power_kw = power_num * 1000
            else:
                power_kw = power_num

        # عد المكونات
        components = ['لوح', 'محول', 'بطارية', 'تركيب', 'كابل', 'جهاز', 'نظام']
        component_count = sum(1 for c in components if c in text.lower())

        base_value = 0

        if power_kw > 0:
            # تقدير التكلفة الفعلية (ألواح + محول + تركيب + شغل)
            # تكلفة حقيقية: $2000-2500 لكل kW (أجهزة + تركيب + شغل)
            cost_per_kw = 2200
            total_cost = power_kw * cost_per_kw
        else:
            # إذا لم تكن الطاقة محددة، استخدم عدد المكونات
            if component_count >= 5:
                total_cost = 800_000  # نظام شامل
            elif component_count >= 3:
                total_cost = 500_000
            else:
                total_cost = 200_000  # نظام صغير

        # تطبيق خفض 35% على التكلفة الإجمالية
        final_value = int(total_cost * 0.65)
        reason = f"طاقة: {power_kw}kW - خفض 35%" if power_kw > 0 else f"مكونات: {component_count} - خفض 35%"
...
def scrape_libyantenders():
    """يجلب مناقصات الطاقة الشمسية عبر API المصادق عليه"""
    s = requests.Session()
...
def scrape_source(src_url, src_name):
    ...
    log(f"   → {len(results)} مناقصة طاقة شمسية مؤهلة")
    return results[:20]
...
def build_email(tenders, part_num=None, total_parts=None):
    ...
    for i, t in enumerate(tenders, 1):
        ...
        sol_html = "<br>".join(
            f"<span style='color:#2F7C31'>• {s}</span>" for s in t["solutions"]
        )
        rows += f"""
        <tr style="background:{bg};vertical-align:top">
          ...
          <td style="padding:11px 10px;border:1px solid #dde;font-size:12px;
                     min-width:130px;color:#2F7C31">
            <strong>{t['value']}</strong>
          </td>
          <td style="padding:11px 8px;border:1px solid #dde;text-align:center;width:55px">
            <a href="{t['link']}" style="background:#2F7C31;color:white;padding:5px 10px;
...
<head>
  ...
  <style>
    @media (max-width: 640px) {{
      ...
      .card-solution {{color: #2F7C31;font-size: 11px;line-height: 1.6;margin-bottom: 8px;}}
      .card-value {{color: #2F7C31;font-weight: bold;margin-bottom: 8px;font-size: 12px;}}
      .card-button {{display: inline-block;background: #2F7C31;color: white;padding: 6px 12px;
...
<body style="font-family:Tahoma,Arial,sans-serif;direction:rtl;background:#f0f4e8;padding:16px;margin:0">
  <div style="max-width:900px;margin:auto;background:white;border-radius:10px;
    ...
    <div style="background:#2F7C31;color:white;padding:18px 22px">
      <h2 style="margin:0;font-size:17px">
        مناقصات الطاقة الشمسية — Alwadi Solar Monitor{part_label}
      </h2>
    ...
    <div style="padding:18px 20px">
      <p style="margin-top:0;font-size:14px">
        ...
        فيما يلي مناقصات الطاقة الشمسية والخلايا الشمسية الجديدة المؤهلة.
        عمود <strong>الحل المقترح</strong> يعطيكم فكرة أولية عن نوع المشروع، وعمود
        <strong>القيمة</strong> إما مذكور في المناقصة أو تقدير بناءً على النوع.
      </p>

      <!-- Desktop Table View -->
      <table class="tender-table" style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="background:#2F7C31;color:white;font-size:12px">
...
{cards_html}      </div>
      ...
      <p style="color:#bbb;font-size:10px;margin-top:14px;
        ...
        Alwadi Communications · تحليل تلقائي لمناقصات الطاقة الشمسية · يُرجى التحقق من الموقع للتفاصيل الكاملة
      </p>
    </div>
  </div>
</body>
...
def send_no_tenders_email():
    ...
    subject = f"No New Solar Tenders — {now_gmt2().strftime('%Y-%m-%d')}"
    html_body = f"""
<html>
<head>
  <meta charset="utf-8">
</head>
<body style="font-family:Tahoma,Arial,sans-serif;direction:rtl;background:#e8f5e9;padding:16px;margin:0">
  <div style="max-width:600px;margin:auto;background:white;border-radius:10px;
              overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.13)">
    <div style="background:#00AA00;color:white;padding:18px 22px">
      <h2 style="margin:0;font-size:17px">
        ☀️ بدون مناقصات طاقة شمسية جديدة
      </h2>
      <p style="margin:6px 0 0;font-size:12px;opacity:0.8">
        {now_gmt2().strftime('%Y-%m-%d %H:%M GMT+2')}
      </p>
    ...
    <div style="padding:18px 20px">
      <p style="margin-top:0;font-size:14px;color:#666">
        ...
        لم يتم العثور على مناقصات طاقة شمسية جديدة خلال آخر فحص.
      </p>
      <p style="color:#999;font-size:12px;border-top:1px solid #eee;padding-top:10px">
        Alwadi Communications · مراقبة مناقصات الطاقة الشمسية التلقائية
      </p>
    </div>
  </div>
</body>
...
def main():
    log("=" * 55)
    log("🔍 بدء الفحص الذكي لمناقصات الطاقة الشمسية...")
    ...
    log(f"   → {sum(1 for t in all_new if t['source']=='Libya Tenders')} مناقصة طاقة شمسية مؤهلة")
    time.sleep(1)
    ...
    if all_new:
        try:
            with open('/home/solutions/.openclaw/solar_monitor_results.json', 'w', encoding='utf-8') as f:
                json.dump(all_new, f, ensure_ascii=False, indent=2)
            log(f"💾 تم حفظ {len(all_new)} مناقصة إلى solar_monitor_results.json")
        except Exception as e:
            log(f"⚠️  خطأ حفظ النتائج: {e}")

    if not all_new:
        log("📭 لا مناقصات طاقة شمسية جديدة")
    else:
        ...
        for idx, group in enumerate(batches, 1):
            ...
            subject = f"☀️ Alwadi Solar Tenders{part} — {len(group)} مناقصة | {now_gmt2().strftime('%Y-%m-%d')}"
        ...
        state["solar_tenders"] = [t["id"] for t in all_new]
